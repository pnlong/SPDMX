...
songs.csv  1 ──<  stems.csv     join key: song_id
   │                  │
   │                  └── path → ./chunk_N/audio/{song_id}/{track}.flac
   │                  └── mid  → ./chunk_N/mid/{song_id}.mid
...
| `path` | str | Release-relative stem **directory** (`./chunk_N/audio/{song_id}`) |
| `mid` | str | Release-relative dense MIDI (`./chunk_N/mid/{song_id}.mid`) |
...
Audio file for a row: `{path}/{track}.flac` (paths already include `./chunk_N/…`).
...
4. Unzip each archive next to the CSVs so paths like `./chunk_0/audio/...` resolve.
