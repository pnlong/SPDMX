...
class YourMT3(pl.LightningModule):
    ...
    def test_step(self, batch, batch_idx, dataloader_idx=0) -> Dict:
        ...
        # SPDMX_PACK: optional multi-file pack from PackedAudioFileDataset
        if isinstance(notes_dict, dict) and notes_dict.get("__packed__"):
            _pack_items = notes_dict["items"]
            _pack_n_segs = notes_dict["n_segs"]
        else:
            _pack_items = [notes_dict]
            _pack_n_segs = [int(audio_segments.shape[0])]

        if batch_idx < 3:
            print(
                f"[SPDMX] test_step batch={batch_idx} n_files={len(_pack_items)} "
                f"n_segs={int(audio_segments.shape[0])} subbsz={bsz} "
                f"packed={bool(isinstance(notes_dict, dict) and notes_dict.get('__packed__'))}"
            )

        # Test pitch shift layer: debug only
        if self.test_pitch_shift_layer is not None and self.test_pitch_shift_semitone != 0:
            for _nd in _pack_items:
                for n in _nd['notes']:
                    if n.is_drum == False:
        ...
        for notes_dict, _n_segs in zip(_pack_items, _pack_n_segs):
            ...
            n_items = _n_segs
            start_secs_file = [32767 * i / 16000 for i in range(n_items)]
            if len(pred_token_array_file[0]) == 0:
                continue
